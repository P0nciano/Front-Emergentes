...
router.post('/', async (req, res) => {
  ...
  const { nome, tipo, codigo = null, descricao = null, preco, quantidade = 0, foto, destaque = false } = valida.data
...
router.put('/:id', async (req, res) => {
  ...
  const { nome, tipo, codigo, descricao, preco, quantidade, foto, destaque } = valida.data

  try {
  const produto = await prisma.produto.update({ where: { id: Number(id) }, data: { nome, tipo, codigo, descricao, preco, quantidade, foto, destaque } })
    res.status(200).json(produto)
  } catch (error) {
    res.status(400).json({ error })
  }
