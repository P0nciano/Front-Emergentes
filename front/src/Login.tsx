...
export default function Login() {
  ...
  async function verificaLogin(data: Inputs) {
    try {
      ...
      // Salva o id do cliente no localStorage se o checkbox 'manter' estiver marcado
      if (data.manter && dados.id) {
        localStorage.setItem("clienteKey", String(dados.id))
      } else {
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(verificaLogin)}>
              ...
              <div className="mb-2 mt-1 text-left">
                <span className="text-gray-700 dark:text-gray-200 mr-2">Ainda não tem Cadastro?</span>
                <Link
                  to="/CadCliente"
                  className="text-blue-700 dark:text-blue-300 font-bold underline hover:text-blue-900 dark:hover:text-blue-100"
                >
                  Cadastre-se
                </Link>
              </div>
